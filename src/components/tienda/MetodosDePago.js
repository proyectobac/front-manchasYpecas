...
const MetodosDePago = ({
    ...
    const [paymentViewMode, setPaymentViewMode] = useState('method');
    ...
    useEffect(() => {
        if (selectedPaymentMethod === 'cash' && !randomPaymentCode) {
            setRandomPaymentCode(generateRandomCode());
        }
    }, [selectedPaymentMethod, randomPaymentCode]);
    ...
    const handleCreditCardChange = (e) => { /* Sin cambios */
        ...
        if (name === 'cardNumber') {
            ...
            formattedValue = nums.length <= 2 ? nums : `${nums.slice(0, 2)}/${nums.slice(2, 4)}`;
        } else if (name === 'cvv') {
            formattedValue = value.replace(/\D/g, '').slice(0, 4);
        }
        setCreditCardInfo(prev => ({ ...prev, [name]: formattedValue }));
    };
    const validateCreditCard = () => { /* Sin cambios */
        if (!creditCardInfo.cardNumber || creditCardInfo.cardNumber.replace(/\s/g, '').length < 16) return 'Número de tarjeta inválido';
        if (!creditCardInfo.cardHolder) return 'Ingresa el nombre del titular';
        if (!creditCardInfo.expiryDate || creditCardInfo.expiryDate.length < 5) return 'Fecha de expiración inválida';
        if (!creditCardInfo.cvv || creditCardInfo.cvv.length < 3) return 'CVV inválido';
        return null;
    };
    ...
    if (paymentViewMode === 'failure') {
        return (
            <div className="bg-white p-8 rounded-lg shadow-xl border max-w-md mx-auto text-center animate-fade-in">
                ...
                <button
                    onClick={() => { setSelectedPaymentMethod(null); setPaymentViewMode('method'); setError(null); }}
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl border animate-fade-in max-w-4xl mx-auto">
            ...
            <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div onClick={() => setSelectedPaymentMethod('creditCard')} className={`${paymentOptionBase} ${selectedPaymentMethod === 'creditCard' ? paymentOptionSelected : paymentOptionUnselected}`}>
                    ...
                    <div onClick={() => setSelectedPaymentMethod('pse')} className={`${paymentOptionBase} ${selectedPaymentMethod === 'pse' ? paymentOptionSelected : paymentOptionUnselected}`}>
                    ...
                    <div onClick={() => setSelectedPaymentMethod('cash')} className={`${paymentOptionBase} ${selectedPaymentMethod === 'cash' ? paymentOptionSelected : paymentOptionUnselected}`}>
            ...
            <div className="flex justify-center mt-8">
                <button
                    onClick={handleProcessPaymentInternal}
                    disabled={!selectedBanco || loadingBancos || isPlacingOrder}
                    className={`w-full sm:w-auto px-10 py-3.5 rounded-lg font-bold text-white shadow-lg flex items-center justify-center gap-2 transition-all duration-200 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-2
                        ${(!selectedBanco || loadingBancos || isPlacingOrder)
                            ? 'bg-gray-400 cursor-not-allowed focus:ring-gray-400'
                            : 'bg-green-600 hover:bg-green-700 focus:ring-green-500'}`}
                >
                    {loadingBancos || isPlacingOrder ? <FaSpinner className="animate-spin text-xl" /> : 'Pagar con PSE'}
                </button>
            </div>
        </div>
    );
