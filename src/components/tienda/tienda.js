...
//     FaAngleLeft, FaAngleRight, FaExclamationTriangle
// } from 'react-icons/fa';

// // ... (categoriasPredefinidas, STOCK_THRESHOLD, carouselImages sin cambios)
...
//     { src: 'https://placehold.co/1200x300/3730A3/FFF?text=Ofertas+Especiales', alt: 'Ofertas especiales', title: 'Ofertas Especiales', subtitle: 'Hasta 50% de descuento' },
//     { src: 'https://placehold.co/1200x300/6366F1/FFF?text=Nuevos+Productos', alt: 'Nuevos productos', title: 'Nuevos Productos', subtitle: 'Últimas novedades' },
//     { src: 'https://placehold.co/1200x300/4F46E5/FFF?text=Envío+Gratis', alt: 'Envío gratis', title: 'Envío Gratis', subtitle: 'En compras superiores a $50.000' },
// ];


// const TiendaCliente = () => {
//     // ... (estados de productos, carrito, loading, error, viewMode, checkoutInfo, isPlacingOrder, etc. sin cambios)
...
//     // Helper para verificar si el usuario está autenticado (ej. revisando localStorage)
//     const isAuthenticated = () => !!localStorage.getItem('token');

//     // Agregar esta nueva función para cargar los datos completos del usuario
//     const loadUserDetails = async (userData) => {
//         try {
//             if (userData && userData.userId) {
//                 const userDetails = await UsuariosService.getUserDetails(userData.userId);
//                 console.log("Datos completos del usuario:", userDetails);
                
//                 // Construir nombre completo usando los datos del usuario
//                 const nombreCompleto = [
//                     userDetails.nombre_usuario || '',
//                     userDetails.apellido || ''
//                 ].filter(Boolean).join(' ').trim();
                
//                 // Actualizar el checkoutInfo con los datos completos
...
//                     direccion: userDetails.direccion || prev.direccion,
//                     ciudad: userDetails.ciudad || prev.ciudad,
//                     tipoDocumento: 'CC', // Por defecto CC
//                     tipoPersona: 'natural' // Por defecto natural
//                 }));
...
//                 localStorage.setItem('userPhone', userDetails.telefono || '');
//                 localStorage.setItem('userDocument', userDetails.documento || '');
//                 localStorage.setItem('userAddress', userDetails.direccion || '');
//                 localStorage.setItem('userCity', userDetails.ciudad || '');
//             }
//         } catch (error) {
...
//     // Modificar el useEffect existente para usar la nueva función
//     useEffect(() => {
//         // Limpiar el carrito al iniciar si no hay productos
...
//                 // Si hay un carrito guardado, verificar que los productos existan
//                 const savedCart = localStorage.getItem('cart');
//                 if (savedCart) {
//                     const parsedCart = JSON.parse(savedCart);
//                     const validCart = parsedCart.filter(item => 
//                         activeProducts.some(p => p.id_producto === item.id_producto)
//                     );
                    
//                     if (validCart.length !== parsedCart.length) {
//                         // Si hay productos inválidos en el carrito, actualizar
...
//             if (userDataString && !pendingOrder) {
//                 try {
...
//         } else {
//             localStorage.removeItem('cart'); // Limpiar si el carrito está vacío
//         }
//     }, [cart]);


//     // ... (fetchProductos, useEffects para carrusel y filtro de productos, nextSlide, prevSlide, formatCurrency, showToast, addToCart, removeFromCart, updateCartQuantity, cartTotal, cartItemCount, handleCheckoutInfoChange SIN CAMBIOS)
...
//                 })));
//             } else {
//                 setProductos([]); setFilteredProductos([]); setError('No se pudieron cargar los productos.');
//             }
//         } catch (err) {
//             setError(err.message || "Error al cargar productos."); setProductos([]); setFilteredProductos([]);
//         } finally {
//             setLoading(false);
//         }
//     }, []);
...
//             } else {
//                 newCart.push({
//                     id_producto: producto.id_producto, nombre: producto.nombre, foto: producto.foto,
//                     precioVenta: parseFloat(producto.precioVenta) || 0, quantity: 1,
...
//     // Lógica para manejar el "Proceder al Pago" desde FormularioCheckout
...
//             Swal.fire('Campos Requeridos', 'Completa todos los campos de envío (*).', 'warning');
//             return;
//         }
...
//             Swal.fire('Campos Adicionales Requeridos', 'Email y Número de Documento son necesarios para el pago.', 'warning');
//             return;
//         }
...
//             Swal.fire('Nombre Incompleto', 'Por favor ingresa nombre y apellido.', 'warning');
//             return;
//         }
...
//                 checkoutInfo: {
//                     ...checkoutInfo,
//                     nombreCompleto: checkoutInfo.nombreCompleto.trim()
//                 }, 
//                 timestamp: Date.now() 
//             };
...
//                 text: 'Necesitas iniciar sesión o registrarte para completar tu compra.',
//                 icon: 'info',
...
//             // Antes de redirigir, podrías guardar el estado actual (carrito, checkoutInfo)
//             // en sessionStorage, por si el usuario cancela en Wompi y vuelve, o si el webhook tarda.
//             // Esto es opcional y depende de cómo quieras manejar esos escenarios.
//             // Ejemplo:
//             // const orderSnapshot = { cart, checkoutInfo, paymentAttempt: { reference: data.referenciaPago, method: paymentMethod }};
//             // sessionStorage.setItem('wompiPaymentAttempt', JSON.stringify(orderSnapshot));
//             window.location.href = redirectUrl;
//             // No se llegará aquí si la redirección es exitosa
...
//                     nombre: item.nombre // Añadido para ResumenCompra
//                 })),
//                 total: cartTotal,
...
//             try {
//                 // Se comenta la lógica de crear venta aquí si el backend la crea después del webhook
//                 // o si ya la creó antes de redirigir a Wompi.
//                 // if (paymentMethod !== 'PSE' || (paymentMethod === 'PSE' && !data.isWompiPreConfirmed)) {
//                 //      const ventaResponse = await VentasService.createVenta(orderDataToSave);
//                 //      // ...
//                 // }

//                 // La actualización de stock y creación de venta generalmente se hará en el backend
//                 // cuando el webhook de Wompi confirme el pago 'APROBADO'.
//                 // Aquí, para pagos simulados o si el backend no lo hace en el webhook, puedes mantenerlo.
//                 // Por ahora, asumimos que el webhook se encarga de la lógica de negocio post-pago.
...
//                 });
//                 setViewMode('success'); 
//                 setCart([]); // Limpiar carrito en el estado de React
//                 localStorage.removeItem('cart'); // Limpiar carrito de localStorage
//                 // No limpiar checkoutInfo aquí, ResumenCompra lo usa
//             } catch (error) {
//                 console.error("Error finalizando orden (simulada/directa):", error);
...
//                 text: data.errorMessage || 'Tu transacción no pudo ser completada. Por favor intenta de nuevo.',
...
//     // ... (renderCategoryBar, renderCarousel, renderProductCard, renderProductsGrid, renderCartFloatingButton SIN CAMBIOS)
//     const renderCategoryBar = () => (
//         <div className="mb-4 bg-white p-4 rounded-lg shadow-md border border-gray-200 sticky top-0 z-30">
//             <h2 className="text-xl font-semibold mb-4 text-gray-700 text-center sm:text-left">Explora Categorías</h2>
//             <Link
//                 to="/login" // O a una ruta de perfil si está logueado
...
//                         <button key={index} onClick={() => setCurrentSlide(index)} className={`h-2 w-6 rounded-full ${index === currentSlide ? 'bg-white' : 'bg-white/40'}`} />
//                     ))}
//                 </div>
//             </div>
//         </div>
...
//                     <img src={producto.foto || defaultProductImage} alt={producto.nombre} className="w-full h-full object-contain p-3 transition-transform duration-300 group-hover:scale-105" onError={(e) => { e.currentTarget.src = defaultProductImage; }} />
...
//         if (error) return <div className="bg-red-100 text-red-700 p-4 rounded-lg text-center my-10"><FaExclamationTriangle size={30} className="mx-auto mb-3" /><h3 className="text-lg font-bold">Error: {error}</h3><button onClick={fetchProductos} className="mt-3 bg-red-600 text-white px-4 py-2 rounded">Reintentar</button></div>;
//         if (filteredProductos.length === 0) return <div className="text-center py-16"><p className="text-gray-600 text-lg">No hay productos en esta categoría.</p></div>;
...
//             className={`fixed bottom-6 right-6 bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-full shadow-lg z-50 ${cart.length > 0 ? 'animate-bounce' : ''}`}
//             aria-label="Ver carrito"
//         >
//             <div className="relative">
//                 <FaShoppingCart size={24} />
//                 {cartItemCount > 0 && (<span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full">{cartItemCount}</span>)}
//             </div>
//         </button>
//     );
...
//                         onProceedToCheckout={() => {
//                             if (cart.length === 0) {
//                                 Swal.fire('Carrito Vacío', 'Añade productos para continuar.', 'info');
//                                 return;
//                             }
//                             setViewMode('checkoutInfo');
//                         }}
...
//                         onInputChange={handleCheckoutInfoChange}
//                         cartTotal={cartTotal}
...
const carouselImages = [
    { src: 'https://res.cloudinary.com/dyvf7tkfd/image/upload/v1715098270/productos/Accesorios/perchera_paseadora_f2_o3n7mg.jpg', alt: 'Ofertas especiales', title: 'Ofertas Especiales', subtitle: 'Hasta 50% de descuento' },
    { src: 'https://res.cloudinary.com/dyvf7tkfd/image/upload/v1715098270/productos/Accesorios/isabelinos_forma_de_dona_v2g01f.jpg', alt: 'Nuevos productos', title: 'Nuevos Productos', subtitle: 'Últimas novedades' },
    { src: 'https://res.cloudinary.com/dyvf7tkfd/image/upload/v1715098267/productos/Snacks/Churros_x_50_und_vphh9y.webp', alt: 'Envío gratis', title: 'Envío Gratis', subtitle: 'En compras superiores a $50.000' },
];
...
const TiendaCliente = () => {
    ...
    useEffect(() => {
        ...
        if (selectedCategoria === 'TODAS') {
            const todosLosProductos = Object.values(productosPorCategoria).flat().filter(p => p.estado === 'Activo');
            setFilteredProductos(todosLosProductos);
        } else {
            setFilteredProductos((productosPorCategoria[selectedCategoria] || []).filter(p => p.estado === 'Activo'));
        }
        ...
        setCart(prevCart => {
            ...
            if (existingItemIndex > -1) {
                ...
                newCart.push({
                    id_producto: producto.id_producto, nombre: producto.nombre, foto: producto.foto,
                    precioVenta: parseFloat(producto.precioVenta) || 0, quantity: 1,
                    stockAvailable: producto.stock, // `stock` del producto original
                    categoria: producto.categoria // Guardar categoría si es útil en el carrito
                });
                showToast(`${producto.nombre} añadido al carrito`);
            }
            return newCart;
    ...
    const handleFinalizeOrder = async (paymentResult) => { /* ... sin cambios ... */ 
        setIsPlacingOrder(true);
        const { success, data, paymentMethod, isRedirect, redirectUrl } = paymentResult;
        ...
        if (success) {
            const orderDataToSave = {
                cliente: checkoutInfo, 
                items: cart.map(item => ({
                    id_producto: item.id_producto, cantidad: item.quantity,
                    precioUnitario: item.precioVenta, nombre: item.nombre
                })),
                total: cartTotal, metodoPago: paymentMethod, 
                estadoPago: 'PAGADO', referenciaPago: data.referenciaPago || `REF-${Date.now()}`
            };
            try {
            ...
            Swal.fire({ icon: 'error', title: 'Error en el Pago', text: data.errorMessage || 'Tu transacción no pudo ser completada.', confirmButtonText: 'Intentar de nuevo' });
            setViewMode('payment'); 
        }
        setIsPlacingOrder(false);
    };
    ...
    const renderProductCard = (producto) => { /* ... sin cambios, pero se asegura que `producto.foto` use la URL completa si es necesario o la relativa del backend */
